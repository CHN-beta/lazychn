...
SLOT="0"
IUSE="X aqua debug gles2-only +glib opengl static-libs +svg utils valgrind mingw"
...
multilib_src_configure() {
	...
	if use mingw
	then
		myopts+=" --enable-win32 --enable-win32-font --enable-quartz=no --enable-quartz-image=no --enable-gtk-doc-html=no"
	fi
